...
# feature_extractor = get_combined_feature_vector()
...
def predict(window):
    ...
    classes = ["Speech", "Silence", "Cough", "Throat-Clearing"] #...
...
def authenticate(sock):
    ...
    if (message == msg_request_id):
        print("Received authentix   cation request from the server. Sending authentication credentials...")
        sys.stdout.flush()
    else:
